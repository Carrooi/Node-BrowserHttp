...
describe 'Http', ->
	...
	describe '#get()', ->
		...
		it 'should send request with data and load them from response', (done) ->
			...
			Http.get('localhost', data: {first: 'first message'}, (response) ->
		...
		it 'should load json data with prefix', (done) ->
			...
			Http.get('localhost', {jsonPrefix: 'while(1); '}, (response) ->
	...
	describe '#post()', ->
		it 'should return an error - cross domain request', (done) ->
			...
			Http.post('localhost', (response, err) ->
